...
- 闭环控制：**使用负反馈，我不直接给你占空比了，而是给控制量的给定值，控制器利用误差来自己调整占空比是多少**，如图
  ...
  **好处：电源大小变化、负载在小范围变化时，占空比自己跟着变化，不用管的**
...
1. PID参数整定方法：
   ...
   **⭐5. 频域补偿法（给予Bode图）本文使用**
   ...
   就是你总结的那套流程：设定穿越频率和相位裕量目标，在Bode图上放置零极点来整形环路，属于频域设计方法。在电力电子中最常用，因为对稳定性指标（相位裕量、增益裕量）控制最直观。
...
2. **频域补偿法流程**
   ...
   3. **PID解耦：**将PID传递函数转化为时间常数型，PID未知量转化为Ks，wz1与wz2
   ...
   4. **结合Bode根据设计指标确定补偿传递函数：**确定零极点与增益大小
   ...
   2. **关于切载后稳定性的理解：**
      ...
      **为什么不是必然稳定**
      ...
      ![image-20260309114015422](C:\Users\23227\AppData\Roaming\Typora\typora-user-images\image-20260309114015422.png)
